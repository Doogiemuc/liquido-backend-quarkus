...
One step of that procedure rests on a definition. Before the victories can be locked in they must be sorted from strongest to weakest, and *strongest* admits two defensible readings. They diverge only in an uncommon case, but the case is worth naming: Section 3.3 does so briefly.
...
### 3.3 Which victories count as stronger
...
It remains a decision a system has to take, and one that leaves no trace in the announced winner — which is why it is worth naming rather than leaving implicit in the source code. Chapter 6 states the choice LIQUIDO has made, and why.
...
Chapters 6 to 8 describe systems that are running. Chapter 9 describes one that is designed but not yet released; it is written in the present tense because it specifies a settled design rather than an aspiration. Where the distinction matters, a claim carries one of three markers:
...
A reader who wants to know only what exists today should read Chapters 6 to 8, and take Chapter 9 as a statement of intent.

## 6. The choices LIQUIDO makes
...
### 6.1 A ranked ballot, counted by Ranked Pairs
...
### 6.2 Which victories count as stronger
...
As Section 3.3 sets out, this choice can only change a result when the pairwise majorities contain a cycle, which is uncommon in practice. It is recorded here because it is invisible in the announced winner, not because it is often decisive.

### 6.3 Hashing, keying and domain separation
...
Every voter-derived value is computed with **HMAC-SHA256** under a server secret, so that nobody holding a list of candidate email addresses can compute the corresponding pseudonyms offline. Polly's key derivation and LIQUIDO's ballot checksum both use explicitly delimited, version-prefixed canonical forms, for the injectivity reason given in Section 5.3. **[Implemented]**

### 6.4 What LIQUIDO does about the limits of Part I

Part I is only worth its length if this section is honest. For each property Section 4.1 requires, the table states what LIQUIDO does today — including the rows where the answer is *nothing*.

| Property or limit (Part I) | LIQUIDO's position today | Where |
|---|---|---|
| Eligibility | A right to vote per team membership; one ballot per voter per poll, enforced by a database constraint rather than an application check | 7.4, 8.3 |
| Ballot secrecy against other voters and admins | **Achieved.** Three derived values with different scopes; a ballot holds no reference to a voter or to a right to vote | 8.3 |
| Ballot secrecy against the operator | **Not achieved.** Pseudonymous only: one server secret reconstructs every link. Requires distributed trust | 8.6, 9.7 |
| Individual verifiability | **Achieved.** The ballot checksum is a reproducible commitment the voter can recompute | 8.4 |
| Universal verifiability | **Achieved for team polls.** The tally can be published and the Ranked Pairs result recomputed independently | 8.6 |
| Receipt-freeness | **Deliberately given up** in Tiers 1 and 2 — it is the direct cost of individual verifiability | 4.2, 8.6 |
| Coercion-resistance | **Not offered.** Out of scope for Tiers 1 and 2; a hard prerequisite for Tier 3 | 9.7 |
| Software independence | **Not achieved.** The result rests on trust in the running code, not on published cryptographic evidence | 9.7 |
| Arrow; Gibbard–Satterthwaite | **Not solvable by any rule.** Ranked Pairs is a defensible compromise, not an optimum, and is manipulable in principle like every other non-dictatorial rule | 4.4, 6.1 |
...
**For Tier 3 that trade is not available.** An election whose outcome binds a population is exactly the setting where coercion and vote-buying dominate. Tier 3 must therefore *acquire* coercion-resistance rather than trade it away, without surrendering verifiability — which is why Section 9.7 exists and why it is long.

**Against the operator, the honest word is pseudonymous, not anonymous.** Against other voters, against a team administrator, and against an attacker holding a database dump without the server secret, LIQUIDO is anonymous. Against whoever holds the secret it is not, and no amount of work inside the current architecture changes that. Section 10.3 sets out precisely who can learn what.

### 6.5 Three tiers, three different trades
...
**What this still does not solve.** Both derivations use the same server secret. An operator holding it can reconstruct every link at any layer. Per-team and per-poll scoping defeat an attacker with the database; they do not defeat an attacker with the key. That is the boundary set out in Section 4.3, and splitting the value does not move it. Moving it is the subject of Section 9.7.
...
Chapter 4, "Which victories count as stronger," is folded into Chapter 3 as Section 3.3. It was a self-contained aside on Ranked Pairs' own sorting step, made obsolete as a standalone chapter once Section 3.2 gave Chapter 3 a place for exactly that kind of detail. Its content and its pointer to Section 6.2 (LIQUIDO's actual choice) are unchanged.

Every later chapter shifts down by one to close the gap: Part I now runs 1 to 5, Part II opens at Chapter 6, and Part III is 10 and 11.
...
The document is also reorganised along a cleaner line. **Part I (Chapters 1 to 5) is now voting theory alone** — what a ballot must guarantee, which guarantees are incompatible, and what no voting rule can achieve — with no claims about any particular system. **Part II opens with a new Chapter 6** stating the decisions LIQUIDO takes against that background, including which of Part I's limits it addresses and which it does not. Statements about LIQUIDO that were previously scattered through the theory chapters have moved there. The three tiers keep their order and structure, renumbered to Chapters 7, 8 and 9.

A short **Section 3.3** covers the two ways of measuring the strength of a pairwise victory — winning votes and margin — and why the difference between them can only affect a result when the pairwise majorities form a cycle.
